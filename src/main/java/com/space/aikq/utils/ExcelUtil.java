...
public class ExcelUtil {
	 ...
	 * @throws Exception
	 */
	public List<List<String>> processOneSheet(String filename, int rowStartIndex) throws Exception {
		try{
...
//			(SheetHandler)parser.getContentHandler();
			SheetHandler tmp = (SheetHandler) parser.getContentHandler();
			tmp.setCurRow(rowStartIndex - 1);
			// Seems to either be rId# or rSheet#
			//InputStream sheet2 = r.getSheet("rId1");
			System.out.println("sheet起始行" + tmp.curRow);
	 ...
	 * @throws Exception
	 */
	public  List<List<String>> processAllSheets(String filename, int rowStartIndex) throws Exception {
		OPCPackage pkg = OPCPackage.open(filename);
		XSSFReader r = new XSSFReader( pkg );
		stylesTable = r.getStylesTable();
		SharedStringsTable sst = r.getSharedStringsTable();
		...
		while(sheets.hasNext()) {
			...
			if (start == 0){
				sheetHandler.setCurRow(rowStartIndex - 1);
			}else{
				sheetHandler.setCurRow(rowStartIndex);
			}
		...
		pkg.close();
		return sheetHandler.getSheetData();
	}
	 ...
	 *
	 * @param filePath
	 * @param flag ture: 解析所有sheet
	 * @return
	 */
	public  List<List<String>> getExcel(String filePath, int rowStartIndex, boolean flag){
		try{
			List<List<String>> tmp;
			if(is2003Excel(filePath)){
				ExcelUtil03 excelUtil03 = new ExcelUtil03();
				tmp = excelUtil03.readExcel(filePath, rowStartIndex, flag);
			}else{
				ExcelUtil example = new ExcelUtil();
				if (flag){
					tmp = example.processAllSheets(filePath, rowStartIndex);
				}else{
					tmp = example.processOneSheet(filePath, rowStartIndex);
				}
			}
			return tmp;
		}catch(Exception e){
	...
	public static void main(String[] args) throws Exception {
		ExcelUtil example = new ExcelUtil();
		String filename = "C:\\Users\\Administrator\\Desktop\\上网人员上网记录20181019101351.xlsx";
		List<List<String>> test = example.getExcel(filename, 1,true);
		System.out.println("记录总数：" + (test.size()-1));
